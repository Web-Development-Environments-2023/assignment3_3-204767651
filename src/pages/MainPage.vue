  ...
  methods: {
    ...
    async getRandomRecipes(new_data = false) {
      ...
      try{
        ...
        this.$root.toast("Input Error", "Couldn't get Random Recipes", "danger");
      }
      return []
    ...
    async getLastViewedRecipes() {
      ...
      try{
        ...
        sessionStorage.setItem("lastViewedRecipes", JSON.stringify(response.data));


        return response.data;
      } catch (error) {
        this.$root.toast("Input Error", "Couldn't get Last Viewed Recipes", "danger");
      }
