<template>
  <div class="container">
    ...
    <FamilyRecipePreview v-for="recipe in recipe_list" :key="recipe.id" :recipe="recipe" />
...
methods:{
  async getFamilyRecipes(){
    try{
      const response = await this.axios.get(this.$root.store.server_domain + "/users/myfamilyrecipes");
